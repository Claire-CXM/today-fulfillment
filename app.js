...
function renderTaskList(tasks) {
  if (!tasks.length) { els.taskList.innerHTML = `<div class="empty-state"><span class="empty-state-mark">今</span><h3>今天还没有正式任务</h3><p>先从下面的建议开始，或新建一个真正想兑现的目标。</p><button class="button primary" type="button" data-action="create">新建第一个任务</button></div>`; return; }
  els.taskList.innerHTML = tasks.map(task => {
    ...
    const plannedActions = task.status === 'planned' ? `<button class="button primary" data-action="start" type="button">开始专注</button><button class="button outline" data-action="nodes" type="button">${task.nodes.length ? '编辑节点' : '拆分节点'}</button><button class="button quiet" data-action="complete" type="button">直接完成</button><button class="text-button" data-action="edit" type="button">编辑</button><button class="text-button delete-button" data-action="delete" type="button">删除</button>` : '';
...
function renderStageRewards() {
  ...
  els.stageRewardCard.innerHTML = `<div><p class="eyebrow">阶段奖励</p><h2>${next ? `距离“${next.reward}”还有 ${Math.max(0, next.minutes - minutes)} 分钟` : '当前阶段奖励已全部解锁'}</h2><p>累计专注 ${minutes} 分钟 · 已解锁 ${state.stageRewardsUnlocked.length}/${milestones.length}</p></div><div class="stage-progress"><span style="width:${progress}%"></span></div>`;
}
